...
import { readFileSync, writeFileSync, existsSync, mkdirSync, readdirSync, opendirSync, unlinkSync, createReadStream } from "node:fs";
import { open as openAsync, stat as statAsync, type FileHandle } from "node:fs/promises";
...
export function pollSessions(runExecFile: typeof execFile = execFile): Promise<CliSessionsResult> {
  return new Promise((resolve) => {
    ...
    runExecFile(OPENCLAW_BIN, args, { ...OPENCLAW_SESSIONS_EXEC_OPTIONS, env: childEnv }, (err, stdout, stderr) => {
      if (err) {
...
export async function tailTranscript(
  ...
  try {
    ...
    for (let pass = 0; pass < 3; pass++) {
      ...
      // Rehash whenever the committed prefix or mtime changed. A real
      // perf fix for issue #217 needs incremental digest state in the cursor.
